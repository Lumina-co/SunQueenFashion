<x-app-layout>
    ...
    <x-slot name="header">
        ...
        <a href="{{ route('season.index') }}" class="border border-red-400 text-red-400 rounded-md py-2 px-3">Retour aux thèmes</a>
    </x-slot>
...
@csrf génère un champ de protection contre les attaques CSRF (Cross-Site Request Forgery) pour sécuriser le formulaire,
 et @method('PUT') spécifie que la méthode HTTP utilisée pour la requête est PUT --}}
            ...
            <div class="py-4">
                ...
                <input class="flex items-center h-8 px-4 w-48 bg-gray-50 mt-2 rounded focus:outline-none focus:ring-2" type="text" id="date_fin" name="date_fin" value="{{ $season->date_fin }}">
            </div>
