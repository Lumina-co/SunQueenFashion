<x-app-layout>
    <x-slot name="header">
        ...
        <a href="{{ route('season.create') }}" class="border bg-red-400 text-black-400 rounded-2xl shadow-lg py-2 px-3">Ajouter un nouvel horaire</a>
    </x-slot>
        ...
        <table class="w-full text-sm text-left text-black dark:text-gray-400">
            ...
            <tbody>
                ...
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <td class="px-6 py-4 bg-blue-200">{{ $season->nom }}</td>
                    <td class="px-6 py-4 bg-yellow-200"> {{ $season->date_début }}</td>
                    <td class="px-6 py-4 bg-orange-200">{{ $season->date_fin }}</td>

                    </td>
                    <td class="px-6 py-4 flex items-center flex justify-center items-center gap-2  bg-gray-200">
                        <a href="{{ route('season.edit', $season) }}" class="rounded-2xl bg-red-200 text-sm text-black py-2 px-3 hover:bg-gray-200 hover:text-black-800 shadow-xl cursor-pointer transition-all duration-200">modifier</a>
